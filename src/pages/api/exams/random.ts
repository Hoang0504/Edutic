...
export default async function handler(
  ...
  try {
    ...
    if (!include_parts || !Array.isArray(include_parts) || include_parts.length === 0) {
      return res.status(400).json({ message: "include_parts is required and must be a non-empty array" });
    }

    if (!difficulty || !["easy", "medium", "hard"].includes(difficulty)) {
      return res.status(400).json({ message: "difficulty is required and must be 'easy', 'medium', or 'hard'" });
    }
