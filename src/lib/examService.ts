...
export async function createExamWithData(
  ...
  try {
    ...
    const exam = await Exam.create(
      {
        ...
        year_of_release:
          examData.exam.year_of_release || new Date().getFullYear(),
        is_published: false,
        created_at: new Date(),
        updated_at: new Date(),
      },
    ...
    const examFolderName = `${examData.exam.title
      .toLowerCase()
      .replace(/\s+/g, "_")}_${examData.exam.year_of_release}`;
    ...
    for (const imageFile of groupImageFiles) {
      ...
      if (match) {
        ...
        const groupQuestions = examData.questions.filter(
          (q) => q.group_id?.toString() === groupId
        );
        if (groupQuestions.length > 0) {
          const minQuestion = Math.min(
            ...groupQuestions.map((q) => q.question_number)
          );
    ...
    for (const imageFile of questionImageFiles) {
      ...
      if (match) {
        ...
        const question = examData.questions.find(
          (q) => q.question_number.toString() === questionNumber
        );
        if (question) {
    ...
    for (const partData of examData.parts) {
      ...
      const part = await Part.create(
        {
          ...
          difficulty_level: partData.difficulty_level as
            | "easy"
            | "medium"
            | "hard",
      ...
      if (
        [1, 2, 3, 4].includes(partData.part_number) &&
    ...
    for (const questionData of examData.questions) {
      ...
      if (questionData.group_id) {
        ...
        if (createdGroups.has(groupKey)) {
          ...
          const groupImages =
            groupImageMap.get(questionData.group_id.toString()) || [];
          const imageUrl =
      ...
      const questionImageUrl =
        questionImageMap.get(questionData.question_number.toString()) || "";
      ...
      questionMap.set(
        `${partNumber}:${questionData.question_number}`,
        question.id
      );
    ...
    for (const answerData of examData.answers) {
      const questionId = questionMap.get(
        `${answerData.part_number}:${answerData.question_number}`
      );
...
export async function addPartToExam(
  ...
  try {
    ...
    const examFolderName = `${exam.title.toLowerCase().replace(/\s+/g, "_")}_${
      exam.year_of_release
    }`;
    ...
    for (let i = 0; i < partData.questions.length; i++) {
      ...
      if (questionImages) {
        const imageFile = questionImages.find(
          (img) =>
        ...
        if (imageFile) {
          const imagePath = await saveQuestionImage(
            imageFile,
            examId,
            partData.part_number,
            questionNumber
    ...
    return {
      ...
      summary: {
        ...
        totalAnswers: partData.questions.reduce(
