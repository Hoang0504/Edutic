...
interface AddUserProps {
  onSubmit: (e: React.FormEvent, formData: { email: string; name: string; role: string; avatarFile?: File; password: string }) => void;
  onCancel: () => void;
}
...
const AddUserPage = ({ onSubmit, onCancel }: AddUserProps) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onSubmit(e, { email, name, role, avatarFile, password });
  };
      ...
      <form className="space-y-4" onSubmit={handleSubmit}>
        ...
        <div>
          ...
          <div className="mt-1 flex items-center space-x-4">
            ...
            <button type="button" onClick={() => document.getElementById('avatar')?.click()} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Upload Avatar</button>
