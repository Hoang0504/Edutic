...
import { MicrophoneIcon, StopIcon, PlayIcon, PauseIcon, ClockIcon } from '@heroicons/react/24/solid';
...
interface SpeakingExamProps {
  parts: Part[];
  onAnswerSubmit: (questionId: number, transcription: string, audioBlob?: Blob) => void;
...
const SpeakingExam: React.FC<SpeakingExamProps> = ({ parts, onAnswerSubmit, onPartComplete, onQuestionSubmit, currentQuestionId, onStartPart }) => {
  ...
  const [currentPhase, setCurrentPhase] = useState<'prepare' | 'respond' | 'review'>('prepare');
  const [responses, setResponses] = useState<{ [key: number]: { transcription: string; audioUrl: string } }>({});
      ...
      questions: [
        {
          ...
          prompt: 'Welcome to the Grand Hotel. Our hotel offers comfortable rooms with modern amenities including free Wi-Fi, air conditioning, and room service. The hotel restaurant serves international cuisine from 6 AM to 10 PM. For reservations or inquiries, please call the front desk.',
          prepareTime: 45,
          responseTime: 45,
          answered: false
        },
        {
          id: 2,
          text: 'Read the text aloud clearly and naturally.',
          prompt: 'The annual company meeting will be held on March 15th in the main conference room. All employees are required to attend. The meeting will begin at 9 AM sharp and is expected to last approximately two hours. Please bring your quarterly reports and any questions you may have.',
          prepareTime: 45,
          responseTime: 45,
          answered: false
        }
      ...
      questions: [
        {
          ...
          prompt: 'Look at the picture and describe what you see. Include details about the people, objects, and activities shown.',
          prepareTime: 45,
          responseTime: 30,
          answered: false
        }
  ...
  const startPreparePhase = () => {
    setCurrentPhase('prepare');
    setIsPreparing(true);
    setPrepareTime(currentQuestion.prepareTime);
    setError('');
  ...
  const handleSubmitResponse = () => {
    ...
    if (currentQuestionIndex < currentPart.questions.length - 1) {
      ...
      const partResponses = currentPart.questions.map(q => ({
        questionId: q.id,
        transcription: responses[q.id]?.transcription || (q.id === currentQuestion.id ? transcription : ''),
        audioUrl: responses[q.id]?.audioUrl || (q.id === currentQuestion.id ? audioURL : '')
      }));
      ...
      <div className="p-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div>
            ...
            {currentQuestion.prompt && (
              <div className="bg-gray-50 p-4 rounded-lg mb-4">
                <h4 className="font-medium text-gray-800 mb-2">Text to read:</h4>
            ...
            {currentQuestion.informationText && (
              <div className="bg-yellow-50 p-4 rounded-lg mb-4">
                ...
                <pre className="text-yellow-700 whitespace-pre-line">{currentQuestion.informationText}</pre>
              </div>
            )}
            ...
            {currentQuestion.imageUrl && (
              <div className="bg-gray-50 p-4 rounded-lg mb-4">
                <h4 className="font-medium text-gray-800 mb-2">Image to describe:</h4>
            ...
            <div className="bg-purple-50 p-4 rounded-lg">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <span className="font-medium text-purple-800">Preparation time:</span>
                  <div className="text-purple-700">{formatTime(currentQuestion.prepareTime)}</div>
                </div>
                <div>
                  <span className="font-medium text-purple-800">Response time:</span>
                  <div className="text-purple-700">{formatTime(currentQuestion.responseTime)}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div>
                  ...
                  <div
                    ...
                    className={`flex-1 text-center py-2 rounded-lg text-sm font-medium ${
                      currentPhase === phase
                        ...
                        : index < ['prepare', 'respond', 'review'].indexOf(currentPhase)
            ...
            {currentPhase === 'prepare' && (
              <div className="text-center">
                <div className="mb-4">
                  ...
                  <h3 className="text-lg font-medium text-gray-800">Preparation Time</h3>
                  {isPreparing ? (
                      ...
                      <>
                        ...
                        {speechSupported && (
                          <span className={`block text-xs mt-1 ${
                            ...
                            {isListening ? '🎤 Speech recognition active' : '⚠️ Speech recognition paused'}
                          </span>
                        )}
                      </>
                    ) : (
            ...
            {currentPhase === 'review' && (
              <div>
                <h3 className="text-lg font-medium text-gray-800 mb-4">Review Your Response</h3>
