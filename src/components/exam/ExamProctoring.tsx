...
const ExamProctoring: React.FC<ExamProctoringProps> = ({
  ...
  const initializeMedia = useCallback(async () => {
    try {
      ...
      onViolation('face', 'Không thể truy cập camera hoặc microphone. Vui lòng cấp quyền và thử lại.');
    }
  }, [onViolation]);
  ...
  return (
    <div className={`fixed top-20 right-4 z-50 ${className}`}>
      {/* Camera Preview */}
      <div className={`bg-white rounded-lg shadow-lg border-2 border-gray-300 overflow-hidden transition-all duration-300 ${
        ...
        {isVideoVisible ? (
          <div className="relative">
            ...
            <div className="bg-gray-800 text-white px-3 py-2 flex justify-between items-center">
              ...
              <div className="flex items-center space-x-2">
                {/* Status indicators */}
                <div className={`w-2 h-2 rounded-full ${isRecording ? 'bg-green-400' : 'bg-red-400'}`} 
                     title={isRecording ? 'Camera đang hoạt động' : 'Camera không hoạt động'} />
                <div className={`w-2 h-2 rounded-full ${faceDetected ? 'bg-green-400' : 'bg-red-400'}`} 
                     title={faceDetected ? 'Đã phát hiện khuôn mặt' : 'Không phát hiện khuôn mặt'} />
                <div className={`w-2 h-2 rounded-full ${audioLevel > 20 ? 'bg-yellow-400' : 'bg-gray-400'}`}
                     title={`Mức âm thanh: ${Math.round(audioLevel)}`} />
                <div className={`w-2 h-2 rounded-full ${modelsLoaded ? 'bg-blue-400' : 'bg-orange-400'}`}
                ...
                <div className="flex-1 bg-gray-300 rounded-full h-1">
                  <div
                    className={`h-1 rounded-full transition-all duration-200 ${
                      audioLevel > 30 ? 'bg-red-500' : audioLevel > 20 ? 'bg-yellow-500' : 'bg-green-500'
                    }`}
          ...
          <button
            ...
            className={`w-full h-full text-white flex items-center justify-center ${
              faceDetected ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'
            }`}
